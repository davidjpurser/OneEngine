...
if(class_exists('AQ_Page_Builder')) {
	...
	********************************************************/
	...
	function aq_field_input_icon($field_id, $block_id, $input, $size = 'full', $type = 'text') {
		$output = '<input type="'.$type.'" id="'. $block_id .'_'.$field_id.'" class="input-'.$size.' show-icon" value="'.$input.'" name="aq_blocks['.$block_id.']['.$field_id.']">';
	...
	/* Select field in*/
	function aq_field_select_in($id, $name, $options, $selected) {
		$options = is_array($options) ? $options : array();
	...
	function aq_field_color_picker_in($id, $name, $color, $default = '') {
		$output = '<div class="aqpb-color-picker">';
	...
	/* Checkbox in */
	function aq_field_checkbox_in($id, $name, $check) {
	...
	/* Media Uploader in*/
	function aq_field_upload_in($id, $name, $media, $media_type = 'image') {
